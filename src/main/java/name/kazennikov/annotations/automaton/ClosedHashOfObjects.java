 ...
 * 
 */
public class ClosedHashOfObjects<E> {
	protected int[] hash;
	...
	protected int put(E o) {
		...
		if (10 * objectsStored > 9 * hash.length) {
			...
			for (int j = 0; j < objectsStored; j++) {
				for (i = getHashCode((E)objects[j]); hash[i] != Constants.NO; i = (i + Constants.hashStep)
						% hash.length)
					;
				hash[i] = j;
			}
	...
	protected int getHashCode(E o) {
		return o.hashCode() % objectsStored;
	}

	protected boolean equal(E o1, E o2) {
		return o1.equals(o2);
	}
}
